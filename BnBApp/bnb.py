...
def generateModelForAirBNB():
    ...
    price_raw = readCSV('data/airbnb_class.csv')
    # print("Read class values")
    prices = removeDollar(price_raw, 'price')
    # print("Read features")
    data_features = readCSV('data/preprocessed.csv')
    vec, data_features_csr = createMatrix(data_features)
    # print("Matrix created!!!")
    reg = ridgeRegression(data_features_csr, prices)
    app.config['vec'] = vec
    app.config['reg'] = reg
...
def ridgeRegression(data_features_csr, prices):
    reg = Ridge(alpha=0.06)
